...
func KubernetesReleaseVersion(version string) (string, error) {
	if ver, err := normalizeBuildVersion(version); err == nil {
		return ver, nil
	}
	...
	if ver, err := normalizeBuildVersion(versionLabel); err == nil {
		return ver, nil
	}
...
func normalizeBuildVersion(version string) (string, error) {
	if !kubeReleaseRegex.MatchString(version) {
		return "", errors.Errorf("string '%s' is not version", version)
	}
...
// Internal helper: resolves version label into normalized version
func resolveVersionLabel(bucketURL, versionLabel string) (string, error) {
	...
	for attempts := 0; attempts < maxVersionLabelResolveAttempts; attempts++ {
		url := fmt.Sprintf("%s/%s.txt", bucketURL, ver)
		...
		if remoteVer, err := normalizeBuildVersion(ver); err == nil {
